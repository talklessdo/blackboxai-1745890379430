...
<form action="{{ route('services.store') }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control @error('complaints') is-invalid @enderror" id="complaints" name="complaints" rows="3" required>{{ old('complaints') }}</textarea>
        @error('complaints')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="mb-3">
        ...
        <input type="number" step="0.01" class="form-control @error('cost') is-invalid @enderror" id="cost" name="cost" value="{{ old('cost') }}" required min="0">
        @error('cost')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
